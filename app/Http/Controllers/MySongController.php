...
class MySongController extends Controller
{
    ...
    public function addMySongs(Request $request){
        print_r($request->input('file_name'));
        $user = Auth::user();
        if($user){
            $mySong = new MySong;
            $mySong->file_name = $request->input('file_name');
            ...
            $mySong->loudness = $request->input('loudness');
            $mySong->energy = $request->input('energy');
            $mySong->class = $request->input('score');
            $mySong->visible = 'YES';

            $mySong->save();

            return redirect('/mySongs')->with('success','Song added');
        }else{
